...
#Heuristic 1 calculating the Manhattan Distance   
def manhattanDistance(neighbor):
        cost=0
        w1=18
        w2=95
        for (block_name, block_coord) in neighbor.blocks.iteritems():
            cost+=w1*(abs(neighbor.hand['location'][0]- neighbor.blocks[block_name][0])+abs(neighbor.hand['location'][1]- neighbor.blocks[block_name][1]))
            cost+=w2*(abs(neighbor.blocks[block_name][0]- neighbor.goal[block_name][0])+abs(neighbor.blocks[block_name][1]- neighbor.goal[block_name][1]))
        return cost

#Heuristic 2 calculating the Euclidean Distance
def euclideanDistance(neighbor):
        cost = 0
        w1=18
        w2=95
        for (block_name, block_coord) in neighbor.blocks.iteritems():
            cost+=w1*((math.sqrt(pow(neighbor.hand['location'][0]- neighbor.blocks[block_name][0],2))+pow(neighbor.hand['location'][1]- neighbor.blocks[block_name][1],2)))
            cost+=w2*((math.sqrt(pow(neighbor.blocks[block_name][0]- neighbor.goal[block_name][0],2))+pow(neighbor.blocks[block_name][1]- neighbor.goal[block_name][1],2)))
        return cost
...
def breadthFirst(queue,visited):
    ...
    global weight #To retrieve the next node in the path and weight will be incremented for each node visited.
    for action in node.allowedActions():
        neighbor = copy.deepcopy(node)
        neighbor.do(action)
        if duplicateWorld(neighbor,visited) is False:
            weight+=1
...
def depthFirst(queue,visited):
    ...
    node=heappop(queue)[1]
    global weight #To retrieve the next node in the path and assigns same weight to all the nodes in the same level.
    weight=-1
    for action in node.allowedActions():
        neighbor = copy.deepcopy(node)
        neighbor.do(action)
...
def bestFirst(queue,visited,heuristic):
    ...
    for action in node.allowedActions():
        ...
        deltaCost = neighbor.do(action)
        if duplicateWorld(neighbor,visited) is False:
            weight = heuristic(neighbor) # Calculates the 
            heappush(queue, (weight, neighbor))
...
def aStar(queue,visited,heuristic):
    ...
    for action in node.allowedActions():
        ...
        deltaCost = neighbor.do(action)
        if duplicateWorld(neighbor,visited) is False:
            weight=heuristic(neighbor)
            totalweight=weight+deltaCost
            heappush(queue,(totalweight,neighbor)) 
...
hs = {manhattanDistance,euclideanDistance} #this is a set, put your heuristic functions in here.
